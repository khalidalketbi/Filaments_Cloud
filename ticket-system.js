(() => {
  ...
  async function loadTickets(){
    ...
    const {data,error}=await db.from('feedback_submissions').select('id,ticket_no,kind,message,status,created_at,updated_at').neq('status','resolved').order('created_at',{ascending:false}).limit(50);
    if(error){box.innerHTML=`<div class="muted">${t('تعذر تحميل التذاكر.','Could not load tickets.')}</div>`;return}
    if(!data?.length){box.innerHTML=`<div class="muted">${t('لا توجد تذاكر مفتوحة.','No open tickets.')}</div>`;return}
  ...
  function inject(){
    ...
    const style=document.createElement('style');style.textContent=`.ticket-thanks{padding:12px;border:1px solid color-mix(in srgb,var(--accent2) 45%,var(--line));background:color-mix(in srgb,var(--accent2) 10%,var(--card2));border-radius:12px;line-height:1.7}.ticket-list{display:grid;gap:8px}.ticket-row{width:100%;text-align:start;background:var(--card2);color:var(--text);border:1px solid var(--line);border-radius:12px;padding:11px;display:grid;grid-template-columns:1fr auto;gap:5px}.ticket-row small{grid-column:1/-1}.ticket-state{font-size:10px;border:1px solid var(--line);border-radius:99px;padding:4px 7px}.ticket-state.in_progress{color:var(--warn)}.ticket-modal{position:fixed;inset:0;background:#000a;display:none;align-items:center;justify-content:center;padding:16px;z-index:80}.ticket-modal.show{display:flex}.ticket-dialog{background:var(--panel);border:1px solid var(--line);border-radius:18px;width:min(760px,100%);max-height:90vh;overflow:auto;padding:16px}.ticket-close{float:inline-end}.ticket-thread{display:grid;gap:8px;margin:15px 0}.ticket-bubble{padding:10px 12px;border-radius:12px;background:var(--card2);border:1px solid var(--line)}.ticket-bubble.admin{background:color-mix(in srgb,var(--accent) 15%,var(--card2))}.ticket-bubble small{display:block;color:var(--muted);margin-top:6px}.ticket-reply{display:grid;gap:8px}`;document.head.appendChild(style);
    const my=document.createElement('div');my.id='myTicketsPanel';my.style.marginTop='18px';my.innerHTML=`<div class="section-title"><h3>${t('🎫 تذاكري المفتوحة','🎫 My Open Tickets')}</h3><button id="refreshTickets" class="btn secondary small" type="button">${t('تحديث','Refresh')}</button></div><div id="myTicketsList" class="ticket-list"></div>`;panel.appendChild(my);
  ...
  function init(){if(inject())return;const obs=new MutationObserver(()=>{if(inject())obs.disconnect()});obs.observe(document.body,{childList:true,subtree:true});setTimeout(loadTickets,1200);setInterval(()=>{if(!document.hidden&&$('myTicketsList'))loadTickets()},10000)}
